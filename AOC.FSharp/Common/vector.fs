[<AutoOpen>]
module AOC.FSharp.Common.vector
...
let inline sqrMagnitude (vector: ^v) : 'u =
    components vector |> Seq.map (fun x -> x * x) |> Seq.reduce (+)

let inline magnitude (vector: 'v & #IVector<_, 'u>) : 'r & #IRootFunctions<'r> =
    vector |> sqrMagnitude |> 'r.CreateChecked |> 'r.Sqrt
